...
updates = lasagne.updates.rmsprop(grads, params, learning_rate=0.00001)
...
for _ in range(total_epoch):
    ...
    for idx_batch in range(num_batch_train):
        ...
        y_label=1-np.int32(np.count_nonzero(x,axis=1)==0)
        cost,result=train_model(x,y_label)
        print 'cost:{},result:{}'.format(cost,result.flatten()[:10])
